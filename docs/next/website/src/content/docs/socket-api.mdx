...
`tab.move` reorders a tab in its current workspace. Set `workspace_id` to move
the tab to another workspace. The destination workspace receives the tab at
`insert_index`. A source workspace must retain at least one tab.

```json
{"id":"req_move_tab","method":"tab.move","params":{"tab_id":"w1:t2","workspace_id":"w2","insert_index":1}}
```

A cross-workspace move keeps internal pane identities, running processes, the
tab label, layout, and focus state. Public tab and pane ids are reassigned for the destination
workspace. Previous public pane ids remain valid aliases.
...
that workspace. A cross-workspace event also includes `previous_tab_id`,
`previous_workspace_id`, and the source workspace's updated `previous_tabs`
list.
