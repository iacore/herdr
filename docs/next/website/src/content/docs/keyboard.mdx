...
To move a tab to another workspace, drag it onto a workspace in the sidebar or
select **Move to workspace…** in its context menu. The workspace picker supports
Up, Down, `j`, `k`, Enter, Escape, and the mouse.

The move-to-workspace action has no default shortcut. You can configure
`move_tab_to_workspace` under `[keys]`.
...
move_tab_to_workspace = "prefix+shift+m"
