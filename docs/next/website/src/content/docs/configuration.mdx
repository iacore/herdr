...
Optional actions are unset by default. Bind them with `prefix+` for prefix-mode behavior, or use an explicit modified chord when you intentionally want a direct shortcut. This includes `move_tab_previous`, `move_tab_next`, and `move_tab_to_workspace`. The first two reorder the active tab. The last action opens the workspace picker.
